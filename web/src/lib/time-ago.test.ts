...
describe('dateToTimeAgo', () => {
  ...
  it('returns "1 minute ago" when the date is 1 minute ago', () => {
    const now = new Date()
    const date = new Date(now.getTime() - 60 * 1000)
    expect(dateToTimeAgo(date)).toEqual('1 minute ago')
  })

  it('returns "2 minutes ago" when the date is 2 minutes ago', () => {
    const now = new Date()
    const date = new Date(now.getTime() - 2 * 60 * 1000)
    expect(dateToTimeAgo(date)).toEqual('2 minutes ago')
  })
